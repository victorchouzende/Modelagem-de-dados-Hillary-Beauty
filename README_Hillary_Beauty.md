...
|---|---|---|
| `id_cliente` | Identificador único do cliente | Único e obrigatório |
| `nome` | Nome completo do cliente | Obrigatório |
| `telefone` | Número utilizado para contato | Obrigatório |
| `data_nascimento` | Data de nascimento | Deve ser válida |
| `data_cadastro` | Data de cadastro | Registrada automaticamente |
| `status_cliente` | Situação do cadastro | Ativo ou Inativo |
...
| Atributo | Descrição | Regra |
|---|---|---|
| `id_profissional` | Identificador único | Único e obrigatório |
| `nome` | Nome da profissional | Obrigatório |
| `telefone` | Telefone da profissional | Deve ser válido quando informado |
| `status_profissional` | Situação profissional | Ativa ou Inativa |
...
| Atributo | Descrição | Regra |
|---|---|---|
| `id_procedimento` | Identificador único | Único e obrigatório |
| `nome` | Nome do procedimento | Obrigatório |
| `descricao` | Descrição do procedimento | Opcional |
| `duracao` | Tempo estimado do procedimento | Obrigatório |
| `valor` | Valor cobrado | Maior ou igual a zero |
| `status_procedimento` | Situação do procedimento | Ativo ou Inativo |
...
| Atributo | Descrição | Regra |
|---|---|---|
| `id_agendamento` | Identificador único | Único e obrigatório |
| `id_cliente` | Cliente relacionado | Obrigatório |
| `id_profissional` | Profissional responsável | Obrigatório |
| `id_procedimento` | Procedimento solicitado | Obrigatório |
| `data_agendamento` | Data do procedimento | Obrigatória |
| `horario_inicio` | Horário de início | Obrigatório |
| `horario_fim` | Horário de término | Compatível com a duração |
| `status_agendamento` | Situação do agendamento | Agendado, Confirmado, Realizado, Cancelado ou Não Compareceu |
...
| Atributo | Descrição | Regra |
|---|---|---|
| `id_atendimento` | Identificador único | Único e obrigatório |
| `id_agendamento` | Agendamento relacionado | Obrigatório |
| `id_cliente` | Cliente atendido | Deve corresponder ao agendamento |
| `id_profissional` | Profissional responsável | Deve corresponder ao agendamento |
| `id_procedimento` | Procedimento realizado | Deve corresponder ao agendamento |
| `data_atendimento` | Data do atendimento | Obrigatória |
| `horario_atendimento` | Horário do atendimento | Compatível com o agendamento |
...
| Atributo | Descrição | Regra |
|---|---|---|
| `id_produto` | Identificador único | Único e obrigatório |
| `nome` | Nome do produto | Obrigatório |
| `descricao` | Características do produto | Opcional |
| `unidade_medida` | Unidade de controle | Ex.: unidade, ml, g |
| `quantidade_atual` | Quantidade disponível | Nunca pode ser negativa |
| `estoque_minimo` | Limite mínimo | Maior ou igual a zero |
| `status_produto` | Situação do produto | Ativo ou Inativo |
...
| Atributo | Descrição | Regra |
|---|---|---|
| `id_movimentacao` | Identificador único | Único e obrigatório |
| `id_produto` | Produto movimentado | Obrigatório |
| `tipo_movimentacao` | Entrada ou saída | Valores controlados |
| `quantidade` | Quantidade movimentada | Maior que zero |
| `data_movimentacao` | Data da movimentação | Obrigatória |
| `motivo` | Justificativa da movimentação | Informado quando necessário |
| `id_atendimento` | Atendimento relacionado | Obrigatório quando a saída estiver ligada ao procedimento |
...
| Atributo | Descrição |
|---|---|
| `id_profissional` | Profissional habilitada |
| `id_procedimento` | Procedimento que pode realizar |
...
# Estrutura Relacional Proposta

Uma representação simplificada das principais tabelas é:
...
CLIENTE
  ...
  └──────────< AGENDAMENTO >────────── PROFISSIONAL
                    │                       │
                    │                       │
                    │                       └──< PROFISSIONAL_PROCEDIMENTO >── PROCEDIMENTO
                    │
                    └────────── 0..1 ────── ATENDIMENTO
                                           │
                                           │
...
# Resumo Geral da Modelagem

A solução proposta transforma o processo atual da Hillary Beauty, que depende principalmente de WhatsApp, Instagram e anotações manuais, em uma estrutura de dados centralizada.
...
```text
             ...
             ┌─────────────┼─────────────┐
             ▼             ▼             ▼
       PROFISSIONAL   PROCEDIMENTO    HORÁRIO
             │
             ▼
       ┌──────────────┐
       │  ATENDIMENTO │
...
Durante o desenvolvimento do projeto de modelagem de banco de dados da Hillary Beauty, foi utilizado o **ChatGPT**, ferramenta de inteligência artificial generativa, como recurso de apoio em diferentes etapas do trabalho.

O ChatGPT foi utilizado como ferramenta de apoio principalmente para organizar as informações fornecidas pelo grupo, estruturar os requisitos do sistema, auxiliar na identificação de entidades, atributos e relacionamentos, definir cardinalidades, organizar regras de negócio e revisar a redação da documentação.

As principais etapas em que o **ChatGPT** foi utilizado foram:
